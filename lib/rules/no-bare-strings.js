...
function isValidConfigObjectFormat(config) {
  for (let key in config) {
    ...
    } else if (key === 'ignoreDefaultAllowList') {
    ...
    switch (configType) {
      ...
      case 'object':
        if (Array.isArray(config)) {
          ...
          let { allowlist = [], ignoreDefaultAllowList } = config;
          let list = ignoreDefaultAllowList ? allowlist : [...DEFAULT_ALLOWLIST, ...allowlist];
          return {
            allowlist: sanitizeConfigArray([...new Set(list)]),
